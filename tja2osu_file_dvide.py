...
def divide_tja(filename, songname = ""):
    if (songname is ""):
        songname = filename[:-4] + ".ogg"
    all_file_list = []
    ...
    zipObj = ZipFile('output.osz', 'w')
    zipObj.write(songname)
	
    for all_ready_file in all_file_list:
        name = all_ready_file[:-4]
        piece = name.rsplit(None, 1)
        name = piece[0] + "[" + piece[1] + "]"
        fout = open("%s.osu" % (name,) ,"w")
        sys.stdout = fout
        module = reload(tja2osu)
        tja2osu.tja2osu("%s" % all_ready_file)
        fout.close()
		
        zipObj.write(name + ".osu")
        os.remove(name + ".osu")
