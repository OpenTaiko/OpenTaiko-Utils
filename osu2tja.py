...
def get_timing_point(str, prev_timing_point=None):
	...
	offset, rawbpmv, beats, _, _, _, is_red, is_ggt = ps[:8] 
	is_ggt = (is_ggt != '0')
	...
	try:
		...
		if int(is_red) == 1:	   # BPM change or SCROLL speed change
			...
			ret["redline"] = False
			ret["offset"] = int(get_real_offset(ret["offset"]))
		else:
			assert False
...
def osu2tja(filename, queueId = 3, diff = 9):
	...
	# If the map is the first in queue
	if (queueId == 3):
		print "TITLE:%s" % title.encode("shift-jis")
		print "SUBTITLE:--%s" % artist.encode("shift-jis") + (('「' + subtitle.encode("shift-jis") + '」より') if subtitle else "")	
		print "WAVE:%s" % audio
...
if __name__ == "__main__":
	...
	# Oni only set
		...
		osu2tja(args[0], 3, args[1])
	
	# Oni + ura set
	elif (len(args) == 4):
		osu2tja(args[0], 3, args[1])
		osu2tja(args[2], 4, args[3])
	
	# Regular set
	elif (len(args) == 8):
		osu2tja(args[0], 3, args[1])
		osu2tja(args[2], 2, args[3])
		osu2tja(args[4], 1, args[5])
		osu2tja(args[6], 0, args[7])
	
	# Regular + ura set
	elif (len(args) == 10):
		osu2tja(args[0], 3, args[1])
		osu2tja(args[2], 4, args[3])
		osu2tja(args[4], 2, args[5])
		osu2tja(args[6], 1, args[7])
		osu2tja(args[8], 0, args[9])
	
	else:
		print("usage : osu2tja [[file] [diff]] ...")
		print("	diff sets : ")
		print("		osu2tja [oni[file] [diff]] (Oni only set)")
		print("		osu2tja [oni[file] [diff]] [ura oni[file] [diff]] (Oni + ura set)")
		print("		osu2tja [oni[file] [diff]] [muzukashii[file] [diff]] [futsuu[file] [diff]] [kantan[file] [diff]] (Regular set)")
		print("		osu2tja [oni[file] [diff]] [ura oni[file] [diff]] [muzukashii[file] [diff]] [futsuu[file] [diff]] [kantan[file] [diff]] (Regular + ura set)")
		exit()
